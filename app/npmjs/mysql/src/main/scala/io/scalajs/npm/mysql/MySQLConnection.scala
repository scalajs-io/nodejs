...
trait MySQLConnection extends IEventEmitter {
  ...
  /**
    ...
    * @example connection.query('INSERT INTO log SET data=?', log, function(err, result) { ... })
    */
  def query(options: MySQLQueryOptions, params: js.Array[js.Any], callback: js.Function): Unit = js.native
...
object MySQLConnection {
  ...
  implicit class ConnectionExtensions(val connection: MySQLConnection) extends AnyVal {
    ...
    @inline
    def executeFuture(query: String): Future[MySQLOkPacket] = {
      promiseWithError1[Error, MySQLOkPacket](connection.query(query, _))
    }

    @inline
    def executeFuture(options: MySQLQueryOptions): Future[MySQLOkPacket] = {
      promiseWithError1[Error, MySQLOkPacket](connection.query(options, _))
    }
