...
/**
  ...
  */
@js.native
trait MySQLPool extends IEventEmitter {
  ...
  def getConnection(callback: js.Function): js.Any = js.native
...
object MySQLPool {
  ...
  /**
    ...
    * @author lawrence.daniels@gmail.com
    */
  implicit class PoolExtensions(val pool: MySQLPool) extends AnyVal {

    @inline
    def getConnectionFuture: Future[MySQLConnection] = promiseWithError1[Error, MySQLConnection](pool.getConnection(_))
    ...
    /**
      ...
      */
    @inline
    def onConnection(callback: js.Function1[MySQLConnection, Any]): pool.type = pool.on("connection", callback)
