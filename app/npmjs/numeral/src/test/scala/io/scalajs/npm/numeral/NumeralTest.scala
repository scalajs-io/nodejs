...
class NumeralTest extends AnyFunSpec {
  ...
  describe("Numeral") {
    ...
    it("Should compute the difference between your numeral object and a value") {
      val number = Numeral(1000)
      val value = 100
      val difference = number.difference(value)
      Assert.equal(difference, 900)
    }
