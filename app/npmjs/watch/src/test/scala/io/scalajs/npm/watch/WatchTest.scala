...
class WatchTest extends FunSpec {
  ...
  describe("Watch") {
    ...
    it("supports watchTree") {
      val options = new WatchOptions(ignoreDotFiles = false)
      Watch.watchTree(root, options, (files: js.Dictionary[Stats], current: Stats, prev: Stats) => {
        files foreach { case (file, stat) =>
          info("file '%s' (%d bytes)".format(file, stat.size.toLong))
        }
      })
