...
object Fs {
  ...
  implicit class FsExtensions(val fs: Fs) extends AnyVal {
    ...
    def accessAsync(path: String) = toFuture[js.Any](fs.access(path, _))

    def accessAsync(path: String, mode: Int) = toFuture[js.Any](fs.access(path, mode, _))
    ...
    def renameAsync(oldPath: String, newPath: String) = toFuture[js.Any](fs.rename(oldPath, newPath, _))

    def statAsync(path: String) = toFuture[StatTime](fs.stat(path, _))

    def writeFileAsync(file: String, data: Buffer, options: FileOutputOptions) = toFuture[js.Any](fs.writeFile(file, data, options, _))

    def writeFileAsync(file: String, data: String, options: FileOutputOptions) = toFuture[js.Any](fs.writeFile(file, data, options, _))

    def writeFileAsync(file: String, data: Buffer) = toFuture[js.Any](fs.writeFile(file, data, _))

    def writeFileAsync(file: String, data: String) = toFuture[js.Any](fs.writeFile(file, data, _))
