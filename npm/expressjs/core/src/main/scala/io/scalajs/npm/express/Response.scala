...
object Response {
  ...
  implicit class HttpResponseExtensions(val response: Response) extends AnyVal {
    ...
    @inline
    def badRequest(json: js.Any): Unit = response.status(400).json(json)

    @inline
    def badRequest(message: String): Unit = response.status(400).send(message)

    @inline
    def badRequest(cause: Throwable): Unit = response.status(400).send(cause.getMessage)

    @inline
    def forbidden(): Unit = response.sendStatus(403)

    @inline
    def forbidden(json: js.Any): Unit = response.status(403).json(json)

    @inline
    def forbidden(message: String): Unit = response.status(403).send(message)

    @inline
    def forbidden(cause: Throwable): Unit = response.status(403).send(cause.getMessage)

    @inline
    def internalServerError(): Unit = response.sendStatus(500)

    @inline
    def internalServerError(json: js.Any): Unit = response.status(500).json(json)

    @inline
    def internalServerError(message: String): Unit = response.status(500).send(message)

    @inline
    def internalServerError(cause: Throwable): Unit = response.status(500).send(cause.getMessage)

    @inline
    def notFound(): Unit = response.sendStatus(404)

    @inline
    def notFound(json: js.Any): Unit = response.status(404).json(json)

    @inline
    def notFound(message: String): Unit = response.status(404).send(message)

    @inline
    def notFound(cause: Throwable): Unit = response.status(404).send(cause.getMessage)
