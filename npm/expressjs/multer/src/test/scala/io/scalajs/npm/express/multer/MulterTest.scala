  ...
  describe("MulterTest") {
    ...
    it("handles multi-part file uploads") {
      ...
      app.post(
        ...
        (req: Request, res: Response, next: js.Function) => {
      ...
      val cpUpload = upload.fields(
        js.Array(new MulterField(name = "avatar", maxCount = 1), new MulterField(name = "gallery", maxCount = 8)))
      app.post(
        "/cool-profile",
        cpUpload,
        (req: Request, res: Response, next: js.Function) => {
